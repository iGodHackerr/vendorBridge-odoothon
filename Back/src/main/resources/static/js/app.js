const VB = {
  ...
  modal: (title, body, footer = '', large = false) => {
    ...
    const overlay = document.createElement('div'); overlay.id = 'modal-root'; overlay.className = 'modal-overlay open';
    overlay.innerHTML = `<div class="modal-box ${large ? 'modal-large' : ''}"><div class="modal-header"><h2>${title}</h2><button class="modal-close" data-close>&times;</button></div><div class="modal-body">${body}</div>${footer ? `<div class="modal-footer">${footer}</div>` : ''}</div>`;
  ...
  confirm: (title, message, onConfirm, danger = false) => {
    const overlay = VB.modal(title, `<p>${message}</p>`, `<button class="btn btn-outline" data-close>Cancel</button><button class="btn ${danger ? 'btn-danger' : 'btn-primary'}" id="confirm-action">Confirm</button>`);
    overlay.querySelector('#confirm-action').addEventListener('click', () => { onConfirm(); overlay.remove(); });
  },
  requireRole: (roles) => {
  ...
  activeVendorForUser: (user) => VendorService.getAll().find(v => v.email.toLowerCase() === user.email.toLowerCase())
};
...
function renderLayout(pageTitle) {
  ...
  const navs = {
    'Procurement Officer': [['dashboard.html','Dashboard','Home'],['vendors.html','Vendors','Vendors'],['rfq.html','RFQs','RFQs'],['comparison.html','Quotation Comparison','Compare'],['purchase-orders.html','Purchase Orders','POs'],['invoices.html','Invoices','Invoices'],['activity-logs.html','Activity Logs','Logs']],
    'Vendor': [['dashboard.html','Dashboard','Home'],['rfq.html','My RFQs','RFQs'],['quotations.html','Submit Quotation','Quotes'],['activity-logs.html','Activity Logs','Logs']],
    'Manager': [['dashboard.html','Dashboard','Home'],['approvals.html','Approvals','Approvals'],['reports.html','Reports','Reports'],['activity-logs.html','Activity Logs','Logs']],
    'Admin': [['dashboard.html','Dashboard','Home'],['vendors.html','Vendor Management','Vendors'],['reports.html','Reports','Reports'],['activity-logs.html','Activity Logs','Logs']]
  };
  const icons = { Home: '🏠', Vendors: '🏢', RFQs: '📄', Compare: '⚖️', Approvals: '✅', POs: '📦', Invoices: '🧾', Logs: '🕒', Reports: '📊', Quotes: '💬' };
  const current = location.pathname.split('/').pop();
  const nav = (navs[user.role] || navs.Admin).map(([href, label, key]) => `<a class="nav-link ${href === current ? 'active' : ''}" href="${href}"><span>${icons[key]}</span><span>${label}</span></a>`).join('');
  document.body.insertAdjacentHTML('afterbegin', `<aside class="sidebar" id="sidebar"><div class="sidebar-logo"><span>VendorBridge</span><button class="sidebar-close" id="sidebarClose">&times;</button></div><nav>${nav}</nav></aside><div class="sidebar-backdrop" id="sidebarBackdrop"></div><header class="app-header"><div class="header-left"><button class="hamburger" id="hamburger">☰</button><div class="page-title">${pageTitle}</div></div><div class="header-right"><div class="notification-wrap"><button class="notification-btn" id="notificationBtn">🔔<span class="notification-count" id="notificationCount">0</span></button><div class="notification-panel" id="notificationPanel"></div></div><div class="user-avatar">${user.name.split(' ').map(p => p[0]).join('').slice(0,2)}</div><span class="user-name">${user.name}</span><span class="role-badge">${user.role}</span><button class="btn btn-outline btn-sm" id="logoutBtn">Logout</button></div></header>`);
  const main = document.querySelector('main'); if (main) main.classList.add('main-content');
  ...
  document.getElementById('hamburger').onclick = () => { sidebar.classList.add('open'); backdrop.classList.add('open'); };
  document.getElementById('sidebarClose').onclick = closeMenu; backdrop.onclick = closeMenu;
  document.getElementById('logoutBtn').onclick = () => { LogService.add(user.id, user.name, user.role, 'User logged out', 'Auth'); StorageService.remove('currentUser'); window.location.href = 'login.html'; };
  renderNotifications();
}
...
function renderNotifications() {
  ...
  document.getElementById('notificationCount').textContent = logs.filter(l => new Date(l.timestamp).getTime() >= dayAgo).length;
  const panel = document.getElementById('notificationPanel');
  panel.innerHTML = LogService.getRecent(5).map(l => `<div class="notification-item"><strong>${l.action}</strong><br><span class="muted">${VB.dateTime(l.timestamp)}</span></div>`).join('') || '<div class="notification-item muted">No recent activity</div>';
...
function seedData() {
  ...
  ].forEach((l, i) => LogService.add(users[i % users.length].id, l[0], l[1], l[2], l[3]));
